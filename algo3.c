...
int main(int argc, char const *argv[])
{
    // ファイル入力
    // ファイル階層は各自で合わせてください
    FILE *ifile = fopen("Model1/dat1/idata","r");
    FILE *ofile = fopen("Model1/dat1/odata","r");
    ...
    while(ml<mc&&mc<mr){ 
        if(min(Edl,Edr,Edc)==Edl){
            ...
            ml = (mll+mc)/2;
            mr = (mc+mrr)/2;
            double Edc = Edl; // Edc計算の無駄を省略
            double Edl = (double)Distance(x,s,L,ml)/(ml+1); 
            double Edr = (double)Distance(x,s,L,mr)/(mr+1); 
        }else if(min(Edl,Edr,Edc)==Edc){
            mll = ml;
            mrr = mr; // mc不変
            ml = (mll+mc)/2;
            mr = (mc+mrr)/2;
            double Edl = (double)Distance(x,s,L,ml)/(ml+1); // Edc計算の無駄を省略
            double Edr = (double)Distance(x,s,L,mr)/(mr+1); 
        }else{
            mll = mc; // mrr不変
            mc = mr;
